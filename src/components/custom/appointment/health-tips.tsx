  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-8">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-gray-800 mb-2">Health & Wellness Tip</h2>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          <div className="flex items-center mb-6">
            ...
            <div>
              <h3 className="text-2xl font-bold text-gray-800 mb-1">{currentTip.title}</h3>
              <div className="flex items-center space-x-3">
                <span className="text-sm text-blue-600 font-medium">{currentTip.category}</span>
                <span
                ...
                >
                  {currentTip.priority.charAt(0).toUpperCase() + currentTip.priority.slice(1)} Priority
                </span>
              </div>
            </div>
          </div>
          <p className="text-gray-700 text-lg leading-relaxed">{currentTip.description}</p>
        </div>
            ...
            {healthTips.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  index === currentTipIndex ? "bg-blue-600" : "bg-gray-300 hover:bg-gray-400"
                }`}
              />
            ))}
          </div>
        ...
        <div className="mt-4">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              ...
              style={{ width: `${((currentTipIndex + 1) / healthTips.length) * 100}%` }}
            />
          </div>
        </div>
      </div>
